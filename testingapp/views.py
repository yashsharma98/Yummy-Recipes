...
from testingapp.forms import registration_form,post_form,comment_form, Updatepro,FileModelForm,Updateview,LocationForm
...
def appearance(request):
    ...
    if request.method == 'POST':
        ...
        elif request.POST.get("dark_form") == 'darkcolorform':
            primary_color = request.POST.get('primary_color')
            user_profile.primary_color = primary_color
        ...
        return render(request, 'testingapp/appearance.html', {'user_profile': user_profile})
...
def reset_colors(request):
    ...
    user_profile.theme = "Light theme"
    user_profile.save()
    return redirect('appearance')
...
def default_color_palette1(request):
    ...
    if request.method == 'POST':
        if request.POST.get("dcp_form_type") == 'dcpcolorform1':
            ...
            return JsonResponse({'dcpprimary_color': dcpprimary_color,'dcpsecondary_color':dcpsecondary_color,
            ...
            return JsonResponse({'dcpprimary_color2': dcpprimary_color2,'dcpsecondary_color2':dcpsecondary_color2,
            'dcptertiary_color2':dcptertiary_color2,'dcpactivelink_color2':dcpactivelink_color2,'dcphover_color2':dcphover_color2})
            ...
            return JsonResponse({'dcpprimary_color3': dcpprimary_color3,'dcpsecondary_color3':dcpsecondary_color3,
        ...
        elif request.POST.get("dcp_form_type") == 'darkcolorform1':
            dcpprimary_color6 = request.POST.get('dcp_primary_color6')
            user_profile.primary_color = dcpprimary_color6

            dcpsecondary_color6 = request.POST.get('dcp_secondary_color6')
            user_profile.secondary_color = dcpsecondary_color6

            dcptertiary_color6 = request.POST.get('dcp_tertiary_color6')
            user_profile.tertiary_color = dcptertiary_color6

            dcpactivelink_color6= request.POST.get('dcp_activelink_color6')
            user_profile.active_link_color = dcpactivelink_color6

            dcphover_color6 = request.POST.get('dcp_hover_color6')
            user_profile.hover_color = dcphover_color6

            user_profile.save()

            return JsonResponse({'dcpprimary_color6': dcpprimary_color6,'dcpsecondary_color6':dcpsecondary_color6,
            'dcptertiary_color6':dcptertiary_color6,'dcpactivelink_color6':dcpactivelink_color6,'dcphover_color6':dcphover_color6})
...
def searchresults_view(request):
    ...
    return render(request,'testingapp/searchresults.html',{'form': form, 'results': results,"user_profiles":user_profiles,
    "query":query,'all_recipes':all_recipes})
...
def customer_render_pdf_view(request,feed,pk, *args, **kwargs):    
    ...
    template_path = 'testingapp/viewpdf.html'
    ...
    if pisa_status.err:
        return HttpResponse('Some error <pre>' + html + '<pre>')
    return response
...
def aa_view(request, api_key, location):
    ...
    if request.method == 'POST':
        ...
        # print('aa view',location)
        res = urllib.request.urlopen('http://api.openweathermap.org/data/2.5/weather?q='+location+'&mode=json&units=metric&appid= ').read()  #Add your api key in appid to get weather data
...
def save_location(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            api_key = ""
            #Add your api key in appid to get weather data 'same api key that used above'
...
def send_welcome_email(email, name):
    ...
    from_email = 'Yummyrecipes@django.com'
...
class postListView(HitCountMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = {"all":all, "brkfst": brkfst,"lnch": lnch,"snack":snack,"dnr":dnr,
...
class dashpostview(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        "next_day_brkfst":next_day_brkfst,"rice_recipe_list":rice_recipe_list,"panner_recipe_list":panner_recipe_list}
        return context
...
def post_post_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and file_form.is_valid():
            ...
            feed_instance.save()
            for f in files:
                file_instance = photo(image=f, feed=feed_instance)
                file_instance.save()
            ...
            title = feed_instance.title
            name = user.first_name
            date_posted = feed_instance.date_post.date()

            if profile.send_email:  # Check if the user has opted in
                subject = f'{title}'
                message = f'Hi {name},\n\nA new recipe "{title}" was posted on {date_posted}.\n\nHappy cooking!'
            ...
            # subject = f'{title}'
            # message = f'Hi {name},\n\nA new recipe "{title}" was posted on {date_posted}.\n\nHappy cooking!'
            # from_email = '"Yummy Recipes" <yummyrecipes@django.com>'
            # recipient_list = [email]

            # # send_mail(subject, message, from_email, recipient_list)
            # email_message = EmailMessage(subject, message, from_email, recipient_list)

            # # Attach the image from FileModelForm
            # image_file = file_form.cleaned_data['image']
            # email_message.attach(image_file.name, image_file.read(), image_file.content_type)
...
def newnavbarnavrail(request):
    ...
    return render(request,'testingapp/newnavbar_navrail.html',{"form": form,"titles":titles})
